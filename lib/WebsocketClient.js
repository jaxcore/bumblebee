...
class BumblebeeWebsocketClient extends Client {
	...
	connect() {
		...
		// click logo, sends exit signal to assistant
		socket.on('exit-assistant', () => {
			this.emit('exit-assistant');
		});
	...
	_applicationBegun(application) {
		this.log('_applicationBegun');
		// when app autostarts an assistant that's not active, this event is used to trigger the application
		this.socket.emit('application-begun');
	}
	
	_systemRequest(dataId, args) {
		return new Promise((resolve, reject) => {
			let id = Math.random().toString().substring(2);
			this.socket.once('system-response-' + id, resolve);
			this.socket.emit('system-request', id, dataId, args);
		});
	}
