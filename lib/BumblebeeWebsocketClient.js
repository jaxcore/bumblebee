...
class BumblebeeWebsocketClient extends Client {
	...
	connect() {
		...
		socket.on('run-application-request', (assistant, runId, args, options) => {
			console.log('run-application-request', assistant, runId, args, options);
			
			this.startApplication(assistant, runId, args, options);
			
			// setTimeout(function() {
			// 	let errValue = null;
			// 	let returnValue = 123456;
			// 	socket.emit('run-application-response-'+assistant+'-'+runId, errValue, returnValue);
			// },2000);
		});
		...
		socket.on('application-autostart', (appId) => {
			console.log('RECEIVED application-autostart', appId);
			const initialArgs = this.state.activeApps[appId].initialArgs;
			console.log('emit application-autostart', appId, initialArgs);
